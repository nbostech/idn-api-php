...
abstract class REST_Controller extends MX_Controller {
    ...
    {
        parent::__construct();
        $this->rest_params = json_decode(trim(file_get_contents('php://input')), true);
    ...
    public function header($key = NULL, $xss_clean = NULL)
    {
        if ($key === NULL)
        {
        ...
        return isset($headers[$key]) ? $this->_xss_clean($headers[$key], $xss_clean) : NULL;
    }
